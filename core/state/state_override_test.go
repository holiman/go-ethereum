...
func TestStateOverride(t *testing.T) {
	...
	for _, c := range cases {
		...
		rootstate, _ := New(root, db)
		stateDb := NewOverrideState(rootstate, c.overrides)
...
func TestRecursiveOverride(t *testing.T) {
	...
	rootstate, err := New(root, db)
	if err != nil {
		t.Fatalf("Failed to initialize state, %v", err)
	}
	stateDb := NewOverrideState(rootstate, overrideA)
	...
	stateDb2 := NewOverrideState(stateDb, overrideB)
	if err != nil {
		t.Fatalf("Failed to initialize state, %v", err)
	}
	...
	stateDb3 := NewOverrideState(stateDb2, overrideC)
	...
	// IF we overwrite the overridden-slot, then, if we read it back, we should
	// get the new value, not any of the old.
	stateDb3.SetState(testAllocAddr, testAllocSlotKey, common.HexToHash("0x112233"))
	have, want := stateDb3.GetState(testAllocAddr, testAllocSlotKey), common.HexToHash("0x112233")
	if have != want {
		t.Fatalf("unexpected storage slot, have: %v, want: %v", have, want)
	}
